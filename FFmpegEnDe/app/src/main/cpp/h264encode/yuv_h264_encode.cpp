...
int H264Encoder::init(const char *inYUV420sp,
    ...
    LogD("AVOutputFormat(pOFmt) name = %s, data_codec = %d, mime_type = %s  ",
            this->pOFmt->name,
            this->pOFmt->data_codec,
            this->pOFmt->mime_type);
...
int H264Encoder::alloc_video_stream() {
    //创建一个新的流
...
int H264Encoder::alloc_video_frame() {
    ...
    LogD("pPicBuf mPictureSize = %d, ",this->mPictureSize);
...
void *_start2(void * pVoid) {
    H264Encoder *yuv2H264Encoder = static_cast<H264Encoder *>(pVoid);
...
void H264Encoder::start() {
    simpleLog();
    pthread_create(&this->mEncode_thread_id,NULL,_start2,this);
}

void H264Encoder::readFrame() {
    size_t len = 0;
    int index = 1;
    while (true) {
        ...
        //PTS
        //pFrame->pts=i;
        pFrame->pts = index * (this->pVStream->time_base.den) /
                ((this->pVStream->time_base.num) * this->fps);
