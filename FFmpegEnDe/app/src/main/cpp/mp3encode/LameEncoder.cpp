...
int LameEncoder::init(const char *mp3SavePath, int sampleRate,
                      int channels, uint64_t bitRate) {
...
int LameEncoder::encode(uint8_t *pcm, int bufferSize) {
    ...
    int ret = 0;
    uint8_t *outmp3 = static_cast<uint8_t *>(malloc
            (bufferSize * 1.25 + 7200));
    if (lame_get_num_channels(lameClient) == 2) {
        ret = lame_encode_buffer_interleaved(lameClient,
                                             reinterpret_cast<short *>(pcm),
                                             bufferSize / 4,
                                             outmp3,
                                             (int) (bufferSize * 1.25) + 7500);
    } else {
        ret = lame_encode_buffer(lameClient,
