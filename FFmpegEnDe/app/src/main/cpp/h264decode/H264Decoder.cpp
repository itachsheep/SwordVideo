...
int H264Decoder::alloc_video_stream_info() {
    //找到视频流 id
    int ret = av_find_best_stream(this->pFormatCtx,AVMEDIA_TYPE_VIDEO,
            -1, -1, 0, 0);
    if (ret < 0) {
...
int H264Decoder::init(const char *inH264Path, const char *outYUVPath,
        int w, int h, int videoRate) {
    this->mWidth = w;
    this->mHeight = h;
...
int H264Decoder::decode(AVPacket *avPacket) {
...
void *_startH264Decode(void *pVoid) {
    simpleLog();
    H264Decoder *decoder = static_cast<H264Decoder *>(pVoid);
...
void H264Decoder::start() {
    simpleLog();
    pthread_create(&this->mTID, NULL, _startH264Decode, this);
}

void H264Decoder::startDecoder() {
    simpleLog();
    int ret = 0;
    while (isStart) {
