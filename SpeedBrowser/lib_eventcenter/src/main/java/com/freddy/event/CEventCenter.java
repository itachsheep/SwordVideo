...
public class CEventCenter {
        ...
        synchronized (LISTENER_LOCK) {
            for (String topic : topics) {
                ...
                if (null == obj) {
                    ...
                    CopyOnWriteArrayList<I_CEventListener> list = new CopyOnWriteArrayList<>();
                    list.add(oldListener);
                    list.add(listener);
                    LISTENER_MAP.put(topic, list);
                } else if (obj instanceof List) {
                    // 有多个监听器
                    CopyOnWriteArrayList<I_CEventListener> listeners = (CopyOnWriteArrayList<I_CEventListener>) obj;
    ...
    public static void dispatchEvent(CEvent event) {
        ...
        if (null != event && !TextUtils.isEmpty(event.getTopic())) {
            ...
            CopyOnWriteArrayList<I_CEventListener> listeners = null;
            ...
            synchronized (LISTENER_LOCK) {
                ...
                if (obj instanceof I_CEventListener) {
                    ...
                    listeners = (CopyOnWriteArrayList<I_CEventListener>) obj;
                }
            }
